...
I used to do all my writing in Bear but I eventually found myself starting to use roam more especially in the initial drafting stages. All my knowledge so far is just two brackets away which I can easily reference if I need to in my main writing doc. The document view which superficially hides bullets is helpful in making it feel like I’m in a dedicated writing app. The toggle feature helps me view structure and top level arguments of the piecemore easily. Because of this, I decided to code themes for roam inspired by Bear - cos I started finding myself using roam to write my essay drafts instead of bear. 

There's little to complain about Roam's function, but its original theme has styling choices that make it uncomfortable to read or write on it for long periods of time. For example, there are not enough spacing between lines and the font size can be too small in different browsers at 100% zoom. The original theme is also high contrast which is harder on the eyes especially at night when it is recommended surrounding environment light should be dimmed to leave the sleep/wake cycle undisturbed. The theme also lacks warmth and joy. These things should be of concern when this app is essentially a writing/research companion. 
...
The CSS can be copied into the [stylus extension](https://chrome.google.com/webstore/detail/stylus/clngdbkpkpeebahjckkjfobafhncgmne).
...
- ### adjustments to increase readability and ease of use
    - ⬜️ I've claimed Solarised and Charcoal have the lowest contrast out of all the themes - actually quantify the contrast level 
    - ⬜️ cannot move down to next block when down arrow is pressed, but can move up. a workaround can be moving to end of the block using right arrow twice, but there shouldn't be a workaround, the original roam app doesn't have this issue
    ...
    - ⬜️ test on different resolutions
        - looks off in 4k - too much spacing between sidebar and main app, maybe some units like width should be percentage based
    - code clean-up 
        - ⬜️ read more about code structuring
            - how to structure and name variables so it's easy to swap out colours for duplicate themes. there's structural things that should stay the same like spacing, but colours should be easily editable. 
        ...
        - ⬜️ do i need so many variations in colours?
...
- ### adjustments for theme consistency
    ...
    - ✅ ~~experiment with mashing up solarised light with olive dunk theme, so changing secondary colour teal to olive green from olive dunk~~
        - thinking green secondary and leaving golden hues for this warm, fresh look that reminds me of a vegetable garden
        - will be called solarised olive dunk theme
...
## cool stuff to add
- ⬜️ automatically switch from light to dark theme as scheduled with user's day/night system setting, also needs to correspond with the light and dark switch in the UI (doesn't work for me atm)
...
- ⬜️ menu to add to the existing Roam toolbar that allows myself to easily make changes directly in the browser e.g. switching between colours for highlighters for example without having to go in and edit the code. the way bear has a menu to let us choose line-height or themes
...
- ⬜️ ticking a checkbox is accompanied by a strikethrough, much more easily differentiated and feels like you've "done" or finished a task
- ⬜️ caret 
...
- ❌ ~~reproduce olive theme from Bear for fresher feel~~
    - on second look, feels too cold and not as warm as solarised joy, so decided to mash the two together 

## What I've learnt from trying to code my own theme

- investigating how to properly sync Git, GitHub, and GitHub Desktop
    - ⬜️ yet to install Git, trying to troubleshoot Homebrew install
...
- have been examining Bear Pro's themes attentively to recreate in Roam and realise attention to detail matters when creating a premium experience